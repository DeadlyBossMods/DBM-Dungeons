...
function mod:SPELL_CAST_START(args)
	...
	if spellId == 260280 then
		...
		local timer = self.vb.stageTotality == 1 and 30 or "v20-25"
		timerGatlingGunCD:Start(timer, self.vb.gatCount+1)
	...
	elseif spellId == 260813 then
		...
		local timer = self.vb.stageTotality == 1 and 30 or "v21-24"
		timerHomingMissileCD:Start(timer, self.vb.homingCount+1)
	end
end
